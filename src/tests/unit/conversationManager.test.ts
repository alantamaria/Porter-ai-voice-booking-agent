...
describe('STEP 4: Assessment Replay Scenarios (Section 31)', () => {
  // Scenario A — Information in arbitrary order
  it('Scenario A: Information in arbitrary order', async () => {
    let state = createInitialBookingState('scenario-a');

    // Turn 1: Date
    const r1 = await processUserTurn({ userUtterance: 'I need a move Saturday', currentState: state }, createMockClient({ date: '2026-09-26', userIntent: 'PROVIDE_INFORMATION' }));
    state = r1.updatedState;
    assert.equal(r1.action.type, 'ASK_FOR_MISSING_INFORMATION');

    // Turn 2: Pickup
    const r2 = await processUserTurn({ userUtterance: 'Pickup is Kakkanad', currentState: state }, createMockClient({ pickup: 'Kakkanad', userIntent: 'PROVIDE_INFORMATION' }));
    state = r2.updatedState;
    assert.equal(r2.action.type, 'ASK_FOR_MISSING_INFORMATION');
    assert.notEqual(r2.action.payload?.targetField, 'pickup');

    // Turn 3: Inventory
    const r3 = await processUserTurn({ userUtterance: 'I have one sofa and two boxes', currentState: state }, createMockClient({ items: [{ name: 'Sofa', quantity: 1 }, { name: 'Box', quantity: 2 }], userIntent: 'PROVIDE_INFORMATION' }));
    state = r3.updatedState;
    assert.equal(r3.action.type, 'ASK_FOR_MISSING_INFORMATION');
    assert.notEqual(r3.action.payload?.targetField, 'items');

    // Turn 4: Drop
    const r4 = await processUserTurn({ userUtterance: 'Drop is Whitefield', currentState: state }, createMockClient({ dropoff: 'Whitefield', userIntent: 'PROVIDE_INFORMATION' }));
    state = r4.updatedState;
    assert.equal(r4.action.type, 'ASK_FOR_MISSING_INFORMATION');
    assert.equal(r4.action.payload?.targetField, 'time');

    // Turn 5: Time -> enters Review!
    const r5 = await processUserTurn({ userUtterance: '10 AM', currentState: state }, createMockClient({ time: '10:00 AM', userIntent: 'PROVIDE_INFORMATION' }));
    state = r5.updatedState;
    assert.equal(r5.action.type, 'PRESENT_REQUIREMENTS_REVIEW');
    assert.match(r5.responseText, /Whitefield/);
  });

  // Scenario B — Correction
  it('Scenario B: Correction acknowledges new value and updates revision history', async () => {
    let state = createInitialBookingState('scenario-b');
    
    // Turn 1: Initial pickup
    const r1 = await processUserTurn({ userUtterance: 'Pickup is Kakkanad', currentState: state }, createMockClient({ pickup: 'Kakkanad', userIntent: 'PROVIDE_INFORMATION' }));
    state = r1.updatedState;
    assert.equal(state.pickup.normalizedLocation, 'Kakkanad');
    ...
    const r2 = await processUserTurn({
      ...
      pickup: 'Vyttila',
      userIntent: 'CORRECTION',
      userCorrectionDetected: { field: 'pickupLocation', oldValueDetected: 'Kakkanad', newValueDetected: 'Vyttila' }
    }));
    state = r2.updatedState;

    assert.equal(state.pickup.normalizedLocation, 'Vyttila');
    assert.equal(r2.action.type, 'ACKNOWLEDGE_CORRECTION');
    assert.match(r2.responseText, /Vyttila/);
    assert.doesNotMatch(r2.responseText, /Kakkanad/);
    assert.ok(state.metadata.revisionHistory.some(r => r.newValue === 'Vyttila'));
  });

  // Scenario C — Ambiguity
  it('Scenario C: Ambiguity asks for usable time rather than inventing one', async () => {
    const state = createInitialBookingState('scenario-c');
    ...
    assert.match(res.responseText, /specific time/i);
    assert.equal(res.updatedState.schedule.parsedTimeSlot, undefined);
  });

  // Scenario D — Vague inventory
  it('Scenario D: Vague inventory asks for concrete items', async () => {
    const state = createInitialBookingState('scenario-d');
    const res = await processUserTurn({
    ...
    assert.equal(res.action.type, 'ASK_FOR_CLARIFICATION');
    assert.equal(res.action.payload?.targetField, 'inventory');
    assert.match(res.responseText, /specific items/i);
    assert.equal(res.updatedState.inventory.items.length, 0);
  });

  // Scenario E — Off-topic
  it('Scenario E: Off-topic preserves booking context and recovers', async () => {
    const state = createInitialBookingState('scenario-e');
    state.pickup.normalizedLocation = 'Kakkanad';
    state.pickup.verified = true;
    ...
    assert.match(res.responseText, /Porter move/i);
  });

  // Scenario F — Review correction
  it('Scenario F: Correction during review returns to gathering/correction without confirming', async () => {
    const state = createInitialBookingState('scenario-f');
    state.pickup.normalizedLocation = 'Kakkanad';
    state.pickup.verified = true;
    state.dropoff.normalizedLocation = 'Edappally';
    state.dropoff.verified = true;
    state.schedule.parsedDate = '2026-09-28';
    state.schedule.parsedTimeSlot = '10:00 AM';
    state.schedule.isValid = true;
    state.inventory.items = [makeTestItem('Sofa')];
    state.phase = 'REQUIREMENTS_REVIEW';
    ...
    const res = await processUserTurn({
      userUtterance: 'Change the drop location to Whitefield',
      currentState: state
    }, createMockClient({
      dropoff: 'Whitefield',
      userIntent: 'CORRECTION',
      userCorrectionDetected: { field: 'dropoffLocation', oldValueDetected: 'Edappally', newValueDetected: 'Whitefield' }
    }));
  ...
  // Scenario G — Confirmation
  it('Scenario G: Valid confirmation only confirms when requirements are fully complete', async () => {
    const state = createInitialBookingState('scenario-g');
    state.pickup.normalizedLocation = 'Kakkanad';
    state.pickup.verified = true;
    state.dropoff.normalizedLocation = 'Whitefield';
    state.dropoff.verified = true;
    ...
    const res = await processUserTurn({
      userUtterance: "Yes, that's correct",
      currentState: state
    }, createMockClient({
